...
namespace Arq.PortalDemo.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : PortalDemoAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<PortalDemoDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{PortalDemoConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{PortalDemoConsts.ConnectionStringName}", constring);
        }
